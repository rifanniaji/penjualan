<?php

use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Here is where you can register web routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| contains the "web" middleware group. Now create something great!
|
*/

Route::get('/', function () {
    return view('welcome');
});

Route::get('/penjualan','PenjualanController@index');
Route::get('/penjualan/tambah','PenjualanController@tambah');
Route::post('/penjualan/store','PenjualanController@store');
Route::get('/penjualan/edit/{id}','PenjualanController@edit');
Route::post('/penjualan/update/{id}','PenjualanController@update');
Route::get('/penjualan/hapus/{id}','PenjualanController@hapus');



////////////////////////////////
Route::get('/penjualan/penjualan_detail/{id}','PenjualanController@penjualan_detail');
Route::get('/penjualan/penjualan_detail_tambah','PenjualanController@penjualan_detail_tambah');
Route::post('/penjualan/detail_store/{id}','PenjualanController@detail_store');
